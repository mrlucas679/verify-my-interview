...
2. Read `CLAUDE.md` (project conventions + tool inventory) and the auto-memory
...
- **Scoped context** — only what that role needs: relevant file paths, the
  conventions section of CLAUDE.md that applies, acceptance criteria.
...
CONTEXT: Read CLAUDE.md at the repo root FIRST — it defines conventions,
...
Maintain `.claude/orchestrator/PROJECT_STATE.md` as the single source of truth,
...
2. **Independent review agent** — a fresh agent (not the author) reads the
   ...
   secrets), privacy/POPIA, edge cases, consistency with CLAUDE.md conventions,
